...
class TopMenu:
    ...
    def insertWebpageTitle(self):
        self.titleLabel = tk.Label(self.mainFrame,text="Cook n' Look", font=(self.mainTitleFont,self.mainTitleFontSize))
        self.titleLabel.place(relx=0.25,rely=0.15)

    def insertLoginRegisterButton(self):
        self.loginRegisterFrame = tk.Frame(self.mainFrame,bg='orange')
        self.loginRegisterFrame.place(relx=0.9,rely=0.5,relwidth=0.1,relheight=0.2)

        self.loginButton = tk.Button(self.loginRegisterFrame,text="Login",command=lambda: switchPages(currentPage,"loginPage"),font=(self.loginButtonFont,self.loginButtonFontSize)).grid(row=0,column=0,padx=self.loginRegisterPadX)
        self.registerButton = tk.Button(self.loginRegisterFrame,text="Register",command=lambda: switchPages(currentPage,"registerPage"),font=(self.loginButtonFont,self.loginButtonFontSize)).grid(row=0,column=1,padx=self.loginRegisterPadX)

    def insertDropDownMenus(self):
        self.mainMenuFrame = tk.Frame(self.mainFrame,bg='red')
...
class HomePage:
    ...
    def __init__(self, tkRoot):
        self.mainFrame = tk.Frame(tkRoot, bg='blue')

        canvas = tk.Canvas(self.mainFrame, bg='blue')

        self.scrollbar = tk.Scrollbar(self.mainFrame, orient="vertical", command=canvas.yview)
        self.scrollable_frame = tk.Frame(canvas, bg='blue')
    ...
    def createMainPage(self):
        ...
        self.bestSellersFrame = tk.Frame(self.scrollable_frame,bg='red')
        self.bestSellersFrame.grid(row=1,column=0,pady=40)
        self.bestSellersItem1 = tk.Canvas(self.bestSellersFrame,width=self.canvasSize,height=self.canvasSize,bg='red').grid(row=0,column=0,padx=80)
        self.bestSellersItem2 = tk.Canvas(self.bestSellersFrame,width=self.canvasSize,height=self.canvasSize,bg='red').grid(row=0,column=1,padx=80)
        self.bestSellersItem3 = tk.Canvas(self.bestSellersFrame,width=self.canvasSize,height=self.canvasSize,bg='red').grid(row=0,column=3,padx=80)
        ...
        self.recommendedFrame = tk.Frame(self.scrollable_frame, bg='red')
        self.recommendedFrame.grid(row=3, column=0,pady=40)
        self.recommendedItem1 = tk.Canvas(self.recommendedFrame, width=self.canvasSize, height=self.canvasSize, bg='red').grid(row=0, column=0, padx=80)
        self.recommendedItem2 = tk.Canvas(self.recommendedFrame, width=self.canvasSize, height=self.canvasSize, bg='red').grid(row=0, column=1, padx=80)
        self.recommendedItem3 = tk.Canvas(self.recommendedFrame, width=self.canvasSize, height=self.canvasSize, bg='red').grid(row=0, column=3, padx=80)
...
class ComputerPartListing:
    ...
    def __init__(self, tkRoot):
        self.mainFrame = tk.Frame(tkRoot, bg='red')
        self.createListing()
...
class ComputerPartsPage:
    ...
    def insertMainFrame(self):
        relativeX = 0.2
        relativeY = 0.2
        self.mainFrame.place(relx=relativeX,rely=relativeY,relwidth=1-relativeX,relheight=1-relativeY)
    ...
    def createMainPage(self):
        ...
        searchResultsLabel.grid(row=0,column=0,columnspan=2,sticky='W',padx=self.searchResultsPadX,pady=self.searchResultsPadY)
        ...
        for i in range(50):
            ...
            self.listOfItemListings[i].mainFrame.grid(row=i+1, column=1,padx=self.listingPadX,pady=self.listingPadY)
